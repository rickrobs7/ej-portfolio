$("document").ready(function() {
  $(".project-name[data-type='javascript']").css("background-color", "#5b6ccf");
  $(".project-name[data-type='htmlcss']").css("background-color", "#2f6e3f");
  $(".project-name[data-type='game']").css("background-color", "#602c70");

  document.querySelector("#html-css").addEventListener("change", function(evt) {
  ...
  document
    ...
    .addEventListener("change", function(evt) {
      updateProjectView("javascript", evt.target.checked);
    });
  ...
  $(".project-item").each(function() {
    var prodName = encodeURIComponent(
      $(this)
        .children("h2")
        .text()
    );
...
function updateProjectView(categoryName, bVisible) {
  ...
  switch (categoryName) {
    case "htmlcsss":
      ...
      break;
    case "javascript":
      dataSelectorVal = "h2[data-type='javascript']";
      break;
    case "game":
